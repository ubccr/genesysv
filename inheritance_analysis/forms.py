...
class PEDFileUploadForm(forms.Form):
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        user_dataset = Dataset.objects.filter(allowed_groups__in=user.groups.all()).distinct(
        ).exclude(id__in=[ele.dataset.id for ele in InheritanceAnalysisRequest.objects.all()])
