...
class InheritanceAnalysisRequestListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super(InheritanceAnalysisRequestListView, self).get_context_data(**kwargs)
...
def inheritance_analysis_request_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            ped_file = TextIOWrapper(request.FILES['ped_file'], encoding='utf-8')
            ...
            if status is True:
                try:
                    ...
                    form.add_error('dataset', 'A request already exists for this dataset!')
                    context={}
                    context['form'] = form
                    inheritance_analysis_requests = InheritanceAnalysisRequest.objects.filter(dataset__in=dataset_ids)
                    context['inheritance_analysis_requests'] = inheritance_analysis_requests
