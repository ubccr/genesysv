...
def validate_ped(ped, dataset):
    ...
    es = elasticsearch.Elasticsearch(
        host=dataset.es_host, port=dataset.es_port)
    result = es.search(index=dataset.es_index_name,
                       doc_type=dataset.es_type_name, body=sample_id_query)
    database_sample_list = [str(ele['key']) for ele in result['aggregations'][
        'values']['values']['buckets']]
    ped_sample_list = [str(ele) for ele in sheet['IND_ID']]

    invalid_samples = sorted(
        list(set(ped_sample_list) - set(database_sample_list)))

    if invalid_samples:
        return((False, 'The following sample name(s) are not in the {}: {}'.format(dataset.description, ', '.join(invalid_samples))))

    # verify 3 members in each family; verify each member is assigned
    # different & correct status
    errors = []
    ...
    for family_tuple in sheet.groupby('FAM_ID'):
        ...
        if len(family_info) != 3:
            errors.append(
                '{} members in family {}, need 3'.format(len(fam), famid))
            continue
        ...
        if mismatched_child:
            errors.append(
                'pat/mat-id in child row doesn\'t match ind-id in pat/mat row for family {}'.format(family_id))
            continue

        if not found_child:
            errors.append(
                'child row not found for family {}'.format(family_id))
            continue
        ...
        family_record[family_id] = [mother['IND_ID'].item(
        ), father['IND_ID'].item(), child['IND_ID'].item()]
...
# per family provided in fr. store results in an overcomplicated data
# structure.
...
def analyse_gene(family_pedigree, res):
    ...
    comp_het = {family_id: {'mom': [], 'dad': [], 'child': []}
                for family_id in family_pedigree.keys()}
    results = {}
    variant_id_map = {}
    ...
    for doc in res['hits']['hits']:
        ...
        sid_gt = {sample['sample_ID']: sample['sample_GT']
        ...
        for family_id, family in family_pedigree.items():
            ...
            if momgt.count('0') == 2 and dadgt.count('0') == 2 and '1' in childgt:
                ...
                # if doc['_id'] in results:
                results[doc['_id']].setdefault(
                    'hom_recess', []).append(family_id)
                # else:
    ...
    for family_id, family in comp_het.items():
        ...
        # for variant in
        # list(set(family['mom']).symmetric_difference(family['dad']))
        for var1 in family['mom']:
            ...
            for var2 in family['dad']:
                ...
                if var1 in family['child'] and var2 in family['child']:
                    ...
                    results[variant_id_map[var1]].setdefault(
                        'comp_het', []).append({family_id: var2})
                    # else:
                    # results[variant_id_map[var1]] = {'comp_het':[{family_id:var2}]}
                    # if variant_id_map[var2] in results:
                    results[variant_id_map[var2]].setdefault(
                        'comp_het', []).append({family_id: var1})
                    # else:
...
def create_update_body(trio_output):
    ...
    for var_type, var_info in trio_output.items():
        if var_type == 'comp_het':
            ...
            denovo_string = '"denovo" : {}'.format(
                str(var_info).replace("'", '"'))

        elif var_type == 'hom_recess':
            homrecess_string = '"hom_recess" : {}'.format(
                str(var_info).replace("'", '"'))

    update_string = ','.join(
        filter(None, [comphet_string, denovo_string, homrecess_string]))
...
def main():
    ...
    ped = '/home/paulspur/trio_analysis_dev/ashkenazim-trio.ped'
