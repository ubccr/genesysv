...
def main():
    ...
    required.add_argument(
        "--labels", help="Cohort labels separated by commas, e.g., \"control, case\" or \"None\" for no label", required=True)
    ...
    recognized_format_fields = list(
        default_vcf_mapping['FORMAT_FIELDS']['nested_fields'])
    ...
    unrecognized_info_fields = sorted(
        list(set(info_fields.keys()) - set(recognized_info_fields)))

    if 'CSQ' in unrecognized_info_fields:
        CSQ_present = True
        unrecognized_info_fields.remove('CSQ')
        CSQ_fields = {
            field for field in recognized_info_fields if field.startswith('CSQ')}
    else:
        CSQ_present = False
    ...
    if 'ANN' in unrecognized_info_fields:
        ...
        ANN_fields = {
            field for field in recognized_info_fields if field.startswith('ANN')}
    else:
        ANN_present = False
    ...
    if unrecognized_info_fields:
        ...
        msg = 'This script will try to determine their data type.'.ljust(
            80, '.')
        print(msg, end="")
        print(WARNING)
    else:
    ...
    msg = '\nParsing VCF file for available FORMAT (sample) fields'.ljust(
        80, '.')
    print(msg, end=" ")
    ...
    msg = '%d FORMAT (sample) fields found in VCF file' % (
        len(list(format_fields)))
    msg = msg.ljust(80, '.')
    print(msg, end="")
    print(OK)

    unrecognized_format_fields = sorted(
        list(set(format_fields.keys()) - set(recognized_format_fields)))
    ...
    # Inspect variants for empty info fields and determine data type for
    # unrecognized info fields
    line_no = 0
    ...
    for line in tqdm(fp, total=no_lines):
        ...
        if unrecognized_format_fields:
            ...
            for sample in samples:
                sample_dict = dict(
                    zip(format_fields_in_line.split(':'), sample.split(':')))
    ...
    msg = '%d of %d INFO fields have data' % (
        len(info_field_with_data), len(list(info_fields)))
    msg = msg.ljust(80, '.')
    print(msg, end="")
    ...
    info_fields_to_skip = list(
        set(recognized_info_fields) - set(info_field_with_data))
    ...
    for field in info_fields_to_skip:
        if default_vcf_mapping['INFO_FIELDS'].get(field):
            vcf_fields['INFO_FIELDS'][
                field] = default_vcf_mapping['INFO_FIELDS'][field]
    ...
    for field in info_field_with_data:
        if default_vcf_mapping['INFO_FIELDS'].get(field):
            ...
            if info.get('is_non_nested_label_field') and args.labels != "None":
                es_field_datatype = default_vcf_mapping[
                    'INFO_FIELDS'][field]['es_field_datatype']
    ...
    for key in default_vcf_mapping['FIXED_FIELDS'].keys():
        if default_vcf_mapping['FIXED_FIELDS'][key].get('label'):
            if args.labels == 'None':
                FIXED_FIELDS[key] = default_vcf_mapping[
                    'FIXED_FIELDS'][key]['no_label']
            else:
                FIXED_FIELDS[key] = default_vcf_mapping[
                    'FIXED_FIELDS'][key]['label']
        else:
    ...
    vcf_fields['FORMAT_FIELDS']['nested_fields'].update(
        unrecognized_format_field_type)
...
# python inspect_vcf.py --n 100000 --index sim --type wes --vcf
# 20170419_SIM_WES_CASE.hg19_multianno.vcf --labels None
