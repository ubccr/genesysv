...
def generate_es_mapping(hostname,
    ...
    for field in non_nested_fields_dict.keys():
        ...
        es_field_text_analyzer = non_nested_fields_dict[field].get('es_field_text_analyzer')
        ...
        if es_field_text_analyzer:
            mapping['properties'][es_field_name]['analyzer'] = es_field_text_analyzer
    ...
    for field in nested_fields_dict.keys():
        ...
        for nested_field in nested_fields.keys():
            ...
            es_field_datatype = nested_fields[nested_field]['es_field_datatype']
            es_field_text_analyzer = nested_fields[nested_field].get('es_field_text_analyzer')


            tmp_nested_dict = {}
            tmp_nested_dict[es_field_name] = {'type': es_field_datatype}
            mapping['properties'][es_nested_path]['properties'][es_field_name] = {'type': es_field_datatype}
            if es_field_text_analyzer:
                mapping['properties'][es_nested_path]['properties'][es_field_name]['analyzer'] = es_field_text_analyzer
...
def main():
    ...
    if not os.path.exists(args.info):
        raise IOError("VCF information file does not exist at location: %s" %(args.info))
...
# python prepare_elasticsearch_for_import.py --hostname 199.109.192.65 --port 9200 --index sim --type wes --labels None --info inspect_output_for_sim_wes.txt
