...
def make_gui(vcf_info_file, mapping_file, type_name, annot):
	...
	variant_quality_related_fields = ['OND', 'ExcessHet', 'RAW_MQ', 'InbreedingCoeff', 'MQRankSum', 'MQ0', 'BaseQRankSum', 'HWP', 'FS', 'FS','ClippingRankSum', 'MQ', 'QD', 'ReadPosRankSum', 'HaplotypeScore', 'VQSLOD', 'SOR']
	...
	if annot == 'vep':
		...
		disease_association_fields = ['PUBMED', 'PHENO']
		conservation_fields = []
		intervar_fields = []
	elif annot == 'annovar':
		...
		pathogenicity_score_fields = { "Exome_region": ['PolyPhen2_score', 'SIFT_score', 'FatHmm_score', 'PROVEAN_score', 'MutAss_score', 
								'EFIN_Swiss_Prot_Score', 'EFIN_HumDiv_Score', 'CADD_Phred_score', 'Carol_score', 'VEST3_rankscore', 
								...
								'CADD_PHRED', 'CADD_RAW', 'CADD13_RawScore',
								'CADD13_PHRED', 'DANN_score', 'DANN_rankscore', 'fathmm-MKL_coding_score', 
								'fathmm-MKL_coding_rankscore', 'Eigen', 'Eigen-raw', 'Eigen-PC-raw'],
		...
		minor_allele_freq_fields = [key for key in mapping if 'gnomAD_' in key or 'ExAC_' in key or '1000g2015aug_' in key or 'esp6500' in key] 
		
		disease_association_fields = ['Associated_disease', 'ICGC_Id', 'ICGC_Occurrence',  'gwasCatalog', 'Tumor_site', 'CLINSIG', 'CLNDBN', 'CLNDSDBID', 'CLNDSDB', 'CLNACC'] 
	...
	sample_related_fields = ['Sample_ID', 'GT', 'PGT', 'PID', 'AD', 'AD_ref', 'AD_alt', 'MIN_DP', 'GQ', 'PGQ', 'PL', 'SB']	
...
#			tooltip = ""
	...
	for key in sample_related_fields:
		if key in sample_es_mapped_fields:
			...
			elif key in ['Sample_ID', 'GT', 'PGT', 'Family_ID', 'Father_ID', 'Mother_ID', 'Sex', 'Phenotype', 'Mother_Genotype', 'Father_Genotype', 'Mother_Phenotype', 'Father_Genotype']:
			...
			if key in keys_in_es_mapping:
				...
				if key in ["RefSeq", "exon_id_rg", "cdna_change_rg", "aa_change_rg", "EnsembleTranscriptID", "exon_id_eg", "cdna_change_eg", "aa_change_eg"]: # these are nested fields
					if key.startswith('exon_id'):
						...
						gui_mapping_gene[key]['filters'][0]['in_line_tooltip'] =  '(i.e. 2/25)'
						gui_mapping_gene[key]['filters'][0]['form_type'] = "ChoiceField"
						gui_mapping_gene[key]['filters'][0]["widget_type"] = "Select"
						...
						gui_mapping_gene[key]['filters'][0]['in_line_tooltip'] =  '(i.e. c.A727G)'
						gui_mapping_gene[key]['filters'][0]['es_filter_type'] = "nested_filter_term"
						...
						gui_mapping_gene[key]['filters'][0]['in_line_tooltip'] = '(i.e. p.T243A)'
						gui_mapping_gene[key]['filters'][0]['form_type'] = "CharField"
						gui_mapping_gene[key]['filters'][0]['es_filter_type'] = "nested_filter_term"
						gui_mapping_gene[key]['filters'][0]['widget_type'] = "TextInput"
					elif key == 'RefSeq':
						gui_mapping_gene[key]['filters'][0]['display_text'] = "RefGene ID"
						gui_mapping_gene[key]['filters'][0]['in_line_tooltip'] = "(i.e. NM_133378)"
						gui_mapping_gene[key]['filters'][0]['widget_type'] = "UploadField"
						gui_mapping_gene[key]['filters'][0]['es_filter_type'] = "nested_filter_terms"
					elif key == 'EnsembleTranscriptID':
						gui_mapping_gene[key]['filters'][0]['display_text'] = "Ensembl transcript ID"
						gui_mapping_gene[key]['filters'][0]['in_line_tooltip'] = "(i.e. ENST00000460472)"
						gui_mapping_gene[key]['filters'][0]['widget_type'] = "UploadField"
						gui_mapping_gene[key]['filters'][0]['es_filter_type'] = "nested_filter_terms"
...
if __name__ == '__main__':
	vcf_info_file = 'sim_control_three_samples_random_10000_vcf_info.json'
	mapping_file = 'scripts/sim_control_3s_mapping.json'
	type_name = 'sim_control_3s_'
	annot = 'annovar'
